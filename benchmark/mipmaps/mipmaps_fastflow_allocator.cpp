...
namespace
{
  ...
  struct Producer : ff_node_t<char, MipMapTask>
  {
    ...
    MipMapTask* svc(char*)
    {
      ...
      for (const auto& f : filenames)
      {
        ...
        {
          MipMapTask* task = ff_new<MipMapTask>();
  ...
  struct MipMap : ff_node_t<MipMapTask, MipMapTask>
  {
    explicit MipMap()
    ...
    MipMapTask* svc(MipMapTask* task)
    {
  ...
  struct Sink : ff_node_t<MipMapTask, char>
  {
    virtual char* svc(MipMapTask* value) override
    {
      m_values.push_back(std::move(value->filename));
      ff_delete(value);
      return GO_ON;
    }
...
void mipmaps_fastflow_allocator(const Params& params, int threads)
{
  ...
  {
    W.push_back(std::unique_ptr<ff_node_t<MipMapTask, MipMapTask> >(make_unique<MipMap>()));
  }
